...
public final class AttributeFactory
{
    ...
    private static RadiusAttribute vsa(long vendor, long type) throws InstantiationException, IllegalAccessException
    {
        ...
        Class<RadiusAttribute> c = null;
             
        if (v != null)
        {
            c = (Class<RadiusAttribute>) v.typeMap.get(type);
        }
    ...
    {
        RadiusAttribute attr = null;
        Class<RadiusAttribute> c = (Class<RadiusAttribute>) attributeMap.get(type);
    ...
    public static class AttributeFactoryPool extends GenericKeyedObjectPool<Long, RadiusAttribute>
    {
        ...
        {
            super(new BaseKeyedPooledObjectFactory<Long, RadiusAttribute>()
            {
                @Override
                public RadiusAttribute create(Long key) throws Exception {
                    RadiusAttribute a = newAttribute(key);
                    a.recyclable = true;
                    a.recycled = false;
                    return a;
                }
                ...
                @Override
                public PooledObject<RadiusAttribute> wrap(RadiusAttribute value) {
                    return new DefaultPooledObject<RadiusAttribute>(value);
                }

                @Override
                public void passivateObject(Long arg0, PooledObject<RadiusAttribute> arg1) throws Exception
                {
                    RadiusAttribute a = arg1.getObject();
                    a.recycled = true;
                }

                @Override
                public void activateObject(Long arg0, PooledObject<RadiusAttribute> arg1) throws Exception
                {
                    RadiusAttribute a = arg1.getObject();
                    a.recycled = false;
                }
    ...
    {
        private Class<VSADictionary> c;
        private Map<Long, Class<?>> typeMap;
        private Map<String, Class<?>> nameMap;

        public VendorValue(Class<VSADictionary> c, LinkedHashMap<Long, Class<?>> t, Map<String, Class<?>> n) { this.c = c; typeMap = t; nameMap = n; }
    ...
    public static boolean loadAttributeDictionary(AttributeDictionary dict)
    {
        ...
        {
            Long id = i.next();
            Class<VSADictionary> c = (Class<VSADictionary>) vendorMap.get(id);
            try
            {
                ...
                VSADictionary vsadict = c.newInstance();
